...
def convex_set_projection(self, project_away_dim):
    ...
    Here, :math:`m = \mathcal{P}.\text{dim} - \text{length}(\text{project_away_dim})`, and
...
def _compute_project_single_point(self, point, p):
    ...
    is not to be called directly. Instead, call `project` method. This functions uses
    ...
    projected_point_on_the_polytope, projection_distance, _ = self.solve_convex_program_with_containment_constraints(
        x,
...
def _compute_support_function_single_eta(self, eta):
    ...
    `support` method. This functions uses solve_convex_program_with_containment_constraints."""
    x = cp.Variable((self.dim,))
    support_vector, negative_support_function_evaluation, _ = self.solve_convex_program_with_containment_constraints(
        x,
...
def solve_convex_program_with_containment_constraints(self, x, objective_to_minimize, cvxpy_args, task_str=""):
    ...
    Notes:
        This function uses :meth:`get_cvxpy_containment_constraints` to obtain the list of CVXPY expressions that form
        the containment constraints on x.
    ...
    """
    try:
        containment_constraints, _ = self.get_cvxpy_containment_constraints(x)
    except ValueError:
    ...
    if problem.status in [cp.OPTIMAL, cp.OPTIMAL_INACCURATE]:
        ...
        # Infeasible is possible when get_cvxpy_containment_constraints returns infeasible constraints but without
        # detection
        return np.nan * np.ones((self.dim,)), np.inf, problem.status
    else:
        # Should never happen!
