...
def test_minimum_volume_ellipsoid():
    ...
    if TESTING_SHOW_PLOTS:
        ...
        P_full_dim.plot(plt.gca(), alpha=0.2)
        E = Ellipsoid(covering_ellipsoid_center, covering_ellipsoid_shape)
        E.plot(method="outer", ax=plt.gca(), color="r", alpha=0.2)
        plt.xlim([-2, 2])
        plt.ylim([-2, 2])
        plt.show()
...
def test_maximum_volume_ellipsoid():
    ...
    if TESTING_SHOW_PLOTS:
        ...
        plt.gca().set_aspect("equal")
        P_full_dim.plot(plt.gca(), alpha=0.2)
        E = Ellipsoid(ellipsoid_center, ellipsoid_shape)
        E.plot(method="inner", ax=plt.gca(), color="r", alpha=0.2)
        plt.xlim([-2, 2])
        plt.ylim([-2, 2])
        plt.show()
